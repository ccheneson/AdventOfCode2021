...
impl Lines {
    ...
    fn generate_coordinates(&self, base: CoordinateBase, from_incl: u16, to_incl: u16) -> Vec<Coordinates> {
        let up = self.start.y <  self.end.y;
        //The below is needed for .rev()
        let (from_incl, to_incl) = if from_incl < to_incl { (from_incl, to_incl) } else { (to_incl, from_incl) };
        match base {
            CoordinateBase::X(value) => {
                if up { 
                    (from_incl..=to_incl).into_iter().map(|p| Coordinates { x: value, y: p }).collect()
                } else {
                    (from_incl..=to_incl).rev().into_iter().map(|p| Coordinates { x: value, y: p }).collect()
                }
            },
            CoordinateBase::Y(value) => {
                if up { 
                    (from_incl..=to_incl).into_iter().map(|p| Coordinates { x: p, y:value }).collect()
                } else {
                    (from_incl..=to_incl).rev().into_iter().map(|p| Coordinates { x: p, y:value }).collect()
                }
            }
        }
    }
...
mod part01 {
    ...
    use crate::day05::{Coordinates, Lines};
